...
/**
 * * Class simulates the monitoring of traffic flow by counting and recognising
 * various types of vehicles
 * - cars, trucks, bikes, and buses - passing through the control point within
 * specified time intervals.
 * 
 * This class aims to provide a realistic representation of traffic data
 * collection,
 * which can be used to analyse traffic patterns, optimize traffic light timing,
 * and identify potential traffic anomalies.
 * 
 * 
 * * Functionalities:
 * 
 * - Real-time Traffic Data Simulation: Simulates the collection of traffic data
 * in real time through periodic scans,
 * where each scan counts the number of different types of vehicles passing
 * through the intersection.
 * - Anomaly Detection: Identifies traffic anomalies during data collection,
 * such as vehicles stopped for an extended
 * period or occupying multiple lanes, which could indicate traffic incidents or
 * unusual conditions.
 * - Configurable Scan Parameters: Allows customization of the traffic scan
 * parameters, including the number of scans
 * per cycle and the duration of each scan, to adapt the data collection process
 * to varying traffic conditions.
 * - Data Reporting: Provides a summary of the collected traffic data and
 * detected anomalies at the end of each scan cycle,
 * offering insights into traffic flow and potential issues at the control
 * point.
 * 
 * Usage:
 * 
 * This class is designed to be instantiated for each traffic control point (
 * traffic light ) where data collection is necessary.
 * 
 */
public class TrafficDataCollector {
	 ...
	 * to Control Centre.
	 * 
	 * Logic uses a while loop to keep rack the number of scans, and uses
	 * Thread.sleep to delay every scan 2 seconds.
	 * This way it simulates a real word scenario to schedule a traffic scan for n
	 * seconds
	 * per n number of times to complete a traffic scan cycle where would be more
	 * appropriate to use a timer.
	 **/
	public void startDataCollector(int numOfTrafficScans, int scanLengthInSeconds) {
		...
		while (numOfTrafficScans > 0) {
			try {
				...
				// System.out.println("Cycle: " + numOfTrafficScans + " - Cars: " + carCounter +
				// ", Trucks: " + truckCounter + ", Bikes: " + bikeCounter + ", Buses: " +
				// busCounter);
	 ...
	 * - Any vehicles stopped for a period of occupying 2 different leans while
